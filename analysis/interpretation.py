...
def detect_doctor_specialties(raw_log_path: str) -> Dict[str, List[str]]:
    """Определяет специализации врачей с учетом лишних пробелов в названиях столбцов."""
    try:
        # Читаем данные с учетом возможных пробелов в названиях столбцов
        raw_df = pd.read_csv(raw_log_path)
        
        # Нормализуем названия столбцов (удаляем лишние пробелы)
        raw_df.columns = raw_df.columns.str.strip()
        
        # Проверяем наличие нужных столбцов (уже без пробелов)
        ...
        # Сопоставление действий с специализациями
        action_to_specialty = {
            'consult': 'Терапевт',
            'surgery': 'Хирург',
            'test': 'Лаборант',
        ...
        for doctor, specs in specialties.items():
            logging.info(f"{doctor}: {specs}")
        
        return specialties
        
    except Exception as e:
        logging.error(f"Ошибка при определении специализаций: {str(e)}")
        # Возвращаем стандартные специализации в случае ошибки
        return {
            'Dr. Anna': ['Терапевт'],
            'Dr. Bob': ['Терапевт'],
            'Dr. Ben': ['Терапевт'],
...
def analyze_doctors_distribution(traces: pd.DataFrame, 
    ...
    """Анализирует распределение врачей по кластерам с визуализацией результатов."""
    # Создаем копию данных и добавляем информацию о кластерах
    traces = traces.copy()
    traces['cluster'] = clusters
    
    # Удаляем записи без кластера (шум)
    ...
    logging.info(f"Определенные специализации: {doctor_specialties}")
    
    # Подготовка данных: разворачиваем списки врачей и добавляем специализации
    doctors_data = []
    for _, row in traces.iterrows():
        for doctor in set(row['doctors']):
            # Нормализуем имя врача (удаляем лишние пробелы)
            doctor_normalized = str(doctor).strip()
            
            # Получаем специализации или используем "Другое"
            specialties = doctor_specialties.get(doctor_normalized, ['Другое'])
            
            # Логируем для отладки
            if specialties == ['Другое'] and doctor_normalized in doctor_specialties:
                logging.warning(f"Врач '{doctor_normalized}' не найден в doctor_specialties")
    ...
    doctors_df.to_csv(os.path.join(output_dir, "doctors_data_debug.csv"), index=False)
    
    # 1. Распределение врачей по кластерам (абсолютные значения)
    ...
    # 1. Топ-15 врачей по частоте (горизонтальный барплот)
    plt.subplot(2, 2, 1)
...
def get_default_specialties() -> Dict[str, List[str]]:
    """Возвращает стандартные специализации врачей"""
    return {
        'Dr. Anna': ['Терапевт'],
        'Dr. Bob': ['Терапевт'],
        'Dr. Ben': ['Терапевт'],
        ...
        'Team 1': ['Хирургическая бригада'],
        'Team 2': ['Хирургическая бригада'],
        # Добавьте других врачей при необходимости
    }